...
class Simulator:
    def __init__(self):
        ...
        for time, BLK_1, BLK_2, BLK_3, BLK_4 in zip(order['time'], order['BLK_1'], order['BLK_2'], order['BLK_3'],
                                                    order['BLK_4']):
    ...
    def cal_prt_mol(self, machine_name):
        ...
        ## PRT 개수와 MOL 개수 구하기
        process_list = []
        for time, event, mol in zip(self.sample_submission['time'], df[f'Event_{machine_name}'],
                                    df[f'MOL_{machine_name}']):
